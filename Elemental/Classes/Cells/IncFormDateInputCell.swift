...
class IncFormDateInputCell: IncFormBindableElementCell {
   ...
   override func configure(with component: IncFormElemental) {
      ...
      guard let element = component as? IncFormDateInput else { fatalError() }
   ...
   override class func contentSize(for element: IncFormElemental, constrainedWidth width: CGFloat) -> CGSize {
      guard let element = element as? IncFormDateInput else { fatalError() }
      let content = element.content
      let config = element.configuration
   ...
   private func _updateInput(with element: IncFormDateInput) {
