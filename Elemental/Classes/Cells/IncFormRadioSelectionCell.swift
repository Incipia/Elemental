//
//  IncFormRadioSelectionCell.swift
//  GigSalad
//
...
class IncFormRadioSelectionCell: IncFormBindableElementCell {
   ...
   override func awakeFromNib() {
      super.awakeFromNib()
      _tapRecognizer.addTarget(self, action: #selector(IncFormRadioSelectionCell.contentViewTapped(recognizer:)))
      contentView.addGestureRecognizer(_tapRecognizer)
   }
   
   override func configure(with component: IncFormElemental) {
      super.configure(with: component)
      guard let element = component as? IncFormRadioSelection else { fatalError() }
   ...
   override class func contentSize(for element: IncFormElemental, constrainedWidth width: CGFloat) -> CGSize {
      guard let element = element as? IncFormRadioSelection else { fatalError() }
      let content = element.content
      let style = element.configuration
...
extension IncFormRadioSelectionCell: RadioViewDelegate {
