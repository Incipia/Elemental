...
class IncFormTextFieldInputCell: IncFormBindableElementCell {
   ...
   override func configure(with component: IncFormElemental) {
      ...
      guard let element = component as? IncFormTextFieldInput else { fatalError() }
   ...
   override class func contentSize(for element: IncFormElemental, constrainedWidth width: CGFloat) -> CGSize {
      guard let element = element as? IncFormTextFieldInput else { fatalError() }
      let content = element.content
      let style = element.configuration
...
extension IncFormTextFieldInputCell {
...
extension IncFormTextFieldInputCell: UITextFieldDelegate {
